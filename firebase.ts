...
let firebaseDb: any;
let usingMock = !isFirebaseConfigured;
...
if (isProduction && !isFirebaseConfigured) {
  console.error("[Meu Consultor IA®] Firebase não configurado. Defina as variáveis VITE_FIREBASE_* na Vercel.");
}
...
const seedDefaultUser = () => {
  ...
  if (!users.find((u: any) => u.uid === defaultTrialUser.uid)) {
    users.push({ ...defaultTrialUser, password: crypto.randomUUID() });
    modified = true;
  }
  const adminIndex = users.findIndex((u: any) => u.email.toLowerCase() === "enilsonlobo32@gmail.com");
  if (adminIndex === -1) {
    users.push({ ...defaultAdminUser, password: crypto.randomUUID() });
    modified = true;
  }
  if (modified) {
    setLocalUsers(users);
...
class MockAuth {
  ...
  async signInWithEmailAndPassword(email: string, pass: string) {
    ...
    const isCorrectPass = user.password === pass;
  ...
  async createUserWithEmailAndPassword(email: string, pass: string) {
    ...
    if (existingUser) {
      throw new Error("Este e-mail já possui uma conta. Use a tela de login ou a recuperação de senha.");
    }
...
class FirebaseAuthAdapter {
  ...
  async signInWithEmailAndPassword(email: string, pass: string) {
    if (isProduction && !isFirebaseConfigured) {
      throw new Error("Autenticação indisponível: configure as variáveis VITE_FIREBASE_* na Vercel.");
    }
  ...
  async createUserWithEmailAndPassword(email: string, pass: string) {
    if (isProduction && !isFirebaseConfigured) {
      throw new Error("Cadastro indisponível: configure as variáveis VITE_FIREBASE_* na Vercel.");
    }
    const cleanEmail = email.trim().toLowerCase();
    const isAllowed = await checkEmailAuthorization(cleanEmail);
    if (!isAllowed) {
  ...
  async sendPasswordResetEmail(email: string) {
    if (isProduction && !isFirebaseConfigured) {
      throw new Error("Recuperação de senha indisponível: configure o Firebase na Vercel.");
    }
    if (usingMock || !firebaseAuth) {
